...
description: 겜코리아 자동화 실행 AI. 3가지 모드(A/B/C)로 파이프라인을 실행한다. 서치팀과 숏츠팀은 독립 운영. 대표 승인 전 업로드 절대 금지.
...
|------|------|------|--------|------|
| **A** | 8분 | ✅ | ❌ | 초기 마커 빠르게 확보 |
| **B** | 30분 | ✅ | ❌ | **기본 운영 모드** · 품질 강화 |
| **C** | 수동 실행 | ❌ | ✅ 5개/회 | 숏츠 콘텐츠 생성 전용 |
...
```
목적: 가능한 많은 역사문화유산 데이터를 빠르게 수집

[1] 서치 AI → 지역 우선순위에 따라 장소 수집
      우선순위: 서울 → 경기/인천/강화 → 부여/공주 → 경주 → 전북/전남 → 강원 → 경남 → 제주
      최소 조건 미충족 시 즉시 제외

[2] 데이터 정리 AI
      → place_id 생성 (중복이면 기존 병합)
      → data_status → complete
      → map_displayable 판정

[3] 카테고리 분류 AI
      → category_main / category_sub / category_detail / tags 결정

[4] 지도 표기 AI
      → 좌표 있음: map_status → ready
      → 좌표 없음: needs_geocoding → true / map_status → geocoding_needed
...
## B모드 — 품질 중심 서치 (기본 운영 모드)

```
목적: 공식 출처 기반 데이터 품질 강화

[1~4] A모드 서치 흐름과 동일

[5] 품질 강화 작업 (A모드에는 없는 추가 단계)
      ① 중복 검토: place_id 기준 기존 데이터와 비교, 병합
      ② 주소 보강: 지번 주소 → 도로명 주소로 교체
      ③ 좌표 확인: 기존 needs_geocoding 항목 재검토
      ④ 카테고리 보강: category_sub·tags 누락 항목 보완
...
```
목적: 기존 데이터를 숏츠 콘텐츠로 전환
서치 없음. heritage_all.json 기존 데이터만 사용.

[1] 콘텐츠 생성 조건 충족 데이터 선별
      heritage_all.json에서 content_status = waiting 인 데이터만 대상
      아래 조건 전부 충족:
      - data_status = complete
...
검수 통과 (created): N개
보관 (archived): N개
생성 항목: 대본·후킹·썸네일·캡션·해시태그

[에러]
발생: N건 (error_log.json 확인)
